...
public class OBJFile {
    ...
    public OBJFile(GLRenderer renderer, final String filename){
        this(renderer.getContext(),filename,null);
    }
    ...
    private void postProcessing(ArrayList<Float> XYZ, ArrayList<Float> UV, ArrayList<Float> Normals, ArrayList<OBJFileMesh> Meshes) {
        ...
        for(int mi=0;mi<Meshes.size();mi++)
        {
            ...
            for(int i=0;i<M.Triangles_XYZ.size();i+=3)
            {
                ...
                if(xyzc>=65533){
                    Log.d("ANGELOS","THE MODEL HAS TOO MANY VERTICES!!!");
                    /*Mesh m=new Mesh();
                    m.keepData(true);
                    m.setXYZ(newXYZ);
                    m.setUV(newUV);
            ...
            if(newXYZ.size()>0) {
                model=new ShadedTexturedModel();
                //Mesh m = new Mesh();
               // m.keepData(true);
                model.setXYZ(ArrayListToFloat(newXYZ));
                model.setUV(ArrayListToFloat(newUV));
                model.setTriangles(ArrayListToShort(Triangles));
                if(newNormals.size()>0) model.setNormals(ArrayListToFloat(newNormals));
                //else model.computeNormals();
    ...
    private float[] ArrayListToFloat(ArrayList<Float> vertices){
    ...
    private short[] ArrayListToShort(ArrayList<Short> vertices) {
        if (vertices == null || vertices.size() == 0) return null;
    ...
    ShadedTexturedModel model;

    public ShadedTexturedModel getModel(){
        return model;
    }
